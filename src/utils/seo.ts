...
export const ROUTE_SEO: Record<string, PageSEOConfig> = {
  ...
  channels: {
    ...
    description: 'Explore the four official YouTube channels and wider UltraOP creator network across YouTube, Twitch, Kick, Instagram, Rooter and Discord.',
    canonicalPath: '/channels/',
    breadcrumbs: [{ name: 'Home', path: '/' }, { name: 'Channels', path: '/channels/' }]
  },
  projects: {
  ...
  setup: {
    ...
    canonicalPath: '/setup/',
    breadcrumbs: [{ name: 'Home', path: '/' }, { name: 'Setup', path: '/setup/' }]
  },
  contact: {
  ...
  mediaKit: {
    ...
    breadcrumbs: [{ name: 'Home', path: '/' }, { name: 'Media Kit', path: '/media-kit/' }]
  },
  privacy: {
...
export function updateDocumentSEO(config: PageSEOConfig) {
  ...
  if (config.schema) {
    if (Array.isArray(config.schema)) schemas.push(...config.schema);
    else schemas.push(config.schema);
  }

  dynamicScript.textContent = schemas.length > 0
